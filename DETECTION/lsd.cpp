...
{std::shared_ptr<Image> I = std::make_shared<Image>();
    ...
    I->data  = new Pixel[w*h];
    //	I->data = (Pixel*)calloc(w*h,sizeof(Pixel));
    return I;
}
...
Pixel GetPixel(std::shared_ptr<Image> I,int i,int j)
{
    //assert(I && i>=0 && i<I->w && j>=0 && j<I->h);
    if ((I->w*j+i)>(I->w*I->h))
        //return I->data[I->w*I->h];
        std::cout<<"erreur erreur"<<std::endl;
    else
        return I->data[I->w*j+i];
}
...
std::shared_ptr<Image> Scale(std::shared_ptr<Image> self, float scaleX, float scaleY) {
    int newWidth = (int)(self->w * scaleX);
    int newHeight = (int)(self->h * scaleY);
    std::shared_ptr<Image> newImage = NouvelleImage(newWidth, newHeight);//, self.PixelFormat);

    for (int x = 0; x < newWidth; x++) {
        for (int y = 0; y < newHeight; y++) {
            float gx = ((float)x) / newWidth * (self->w - 1);
            float gy = ((float)y) / newHeight * (self->h - 1);
            int gxi = (int)gx;
            int gyi = (int)gy;
            Pixel c00 = GetPixel(self,gxi, gyi);
            Pixel c10 = GetPixel(self,gxi + 1, gyi);
            Pixel c01 = GetPixel(self,gxi, gyi + 1);
            Pixel c11 = GetPixel(self,gxi + 1, gyi + 1);
            Pixel pix;
            for(int k=0; k<3; k++)
            {pix.rgb[k]=(int)Blerp(c00.rgb[k], c10.rgb[k], c01.rgb[k], c11.rgb[k], gx - gxi, gy - gyi);


            }
            SetPixel(newImage,x, y, pix);
        }
    }
    return newImage;
}
...
{std::cout<<"le fichier est"<<fichier<<std::endl;
    std::vector<unsigned char> image; //the raw pixels
    unsigned width, height;
    unsigned error = lodepng::decode(image, width, height, fichier);
    std::shared_ptr<Image> I;
    I = NouvelleImage(width,height);

    for(int i=0;i<width*height;i++)
    {
        I->data[i].rgb[0] = image[4*i];
        I->data[i].rgb[1]= image[4*i+1];
        I->data[i].rgb[2] = image[4*i+2];
...
int Sauver(std::shared_ptr<Image> I,const char* fichier)
{
    ...
    {
        image.push_back((unsigned char)I->data[i].rgb[0]);
        image.push_back((unsigned char)I->data[i].rgb[1]);
        image.push_back((unsigned char)I->data[i].rgb[2]);
        image.push_back((unsigned char)255);
    }
    unsigned error = lodepng::encode(png, image, I->w, I->h);
...
std::shared_ptr<Image> rgbtogray(std::shared_ptr<Image> Im){
    ...
    for(int i=0; i<Im->w; i++){
        for(int j=0; j<Im->h; j++){
            ...
            int grey = (int)pix.greylevel();
            SetPixel(newImage,i,j,Pixel(grey, grey, grey));
        }
    }
    std::cout<<newImage->h<<std::endl;
    return newImage;
}
        ...
        for(int j=0; j <height; ++j) {          // columns
            ...
            for(int m=0; m < filterHeight; ++m) {     // kernel rows
                ...
                for(int n=0; n < filterWidth; ++n) { // kernel columns
                    int nn = filterWidth - 1 - n;  // column index of flipped kernel
                    for(int d=0; d<3; d++){
                                ...
                                double density_th)
{
    ...
    {
        rad *= 0.75;//0.75; /* reduce region's radius to 75% of its value */
